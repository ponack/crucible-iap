...
func (s *Server) registerRoutes(store *storage.Client, q *queue.Client, policyHandler *policies.Handler, v *vault.Vault, n *notify.Notifier, oidc *oidcprovider.Provider) {
	...
	runHandler := runs.NewHandler(s.pool, s.cfg, q, store, n)
...
func (s *Server) registerRunRoutes(
	...
	api.GET("/stacks/:id/plan-diff", runHandler.GetPlanDiff)
	api.GET("/runs/:id/policy-results", runHandler.PolicyResults)
	api.GET("/runs/:id/scan-results", runHandler.ScanResults)
	api.GET("/runs/:id/cost-resources", runHandler.CostResources)
	api.GET("/runs/:id/chain", runHandler.ChainStatus)
	api.POST("/runs/:id/retrigger", runHandler.Retrigger, member)
...
func (s *Server) registerAgentRoutes(
	...
	internal.POST("/runs/:id/plan-json", runHandler.UploadPlanJSON)
	...
	internal.POST("/runs/:id/cost-resources", runHandler.ReportCostResources)
