...
- [ ] Monorepo path filters on triggers — per-stack include / exclude glob list evaluated against the changed-file set in push and PR webhook payloads (`apps/foo/**`, `!**/*.md`); runs only fire when at least one path matches; default behaviour unchanged when no filter is set; closes the biggest current gap vs Digger, Atlantis, and Scalr for teams running monorepos
- [ ] Sequential approver chains — beyond single-approval gating, define ordered approver groups on the stack or via an approval policy (e.g. `tech-lead → director`); the next approver is notified only after the previous one approves; run detail surfaces remaining approvers and elapsed time per step; complements the v0.9.0 approval escalation
- [ ] Webhook push policies — server-side rule engine that filters incoming webhook events before they create a run (branch name patterns, file-path globs, commit-message tags, author / actor filters); reduces queue noise from docs-only commits, branch-creation events, and dependabot PRs without touching the VCS-side filters
- [ ] Worker pool auto-scaling — agent pool scales horizontally based on queue depth; configurable min / max agents and scale thresholds per pool; Docker Compose + Kubernetes recipes; metric-driven scale-up at a configurable queue-depth threshold (defaults safe for small deployments)
- [ ] Per-project and per-team cost quotas — extends the v0.9.0 concurrent-run quota with monthly Infracost-sum caps; per-project (and per-tag) thresholds with `warn` / `block` enforcement modes; pairs with the existing per-stack budget threshold for full FinOps coverage
- [ ] Compliance evidence export — downloadable bundle for SOC 2 / HIPAA / PCI audits: signed run artifacts, audit log slice for the period, policy evaluation records, approval chain records, attached compliance packs and their evaluation history; CSV + JSON + signed manifest; UI at **Settings → Compliance → Export**
- [ ] SBOM + cosign attestation for every release — CycloneDX SBOM generated and attached to each GitHub release; cosign-signed attestation linking the SBOM to the runner image digest; closes the supply-chain transparency gap with commercial competitors and pairs with the existing image-digest pin in `RUNNER_DEFAULT_IMAGE`
- [ ] Multi-stack DAG orchestration — beyond linear `stack_dependencies`: conditional triggers (run downstream only when an upstream output matches a predicate), fan-out (one apply triggers many), fan-in (downstream waits on all upstreams), per-edge retry policy, visual graph editor; the single biggest workflow-feature gap vs Spacelift's "Run Configurations"
- [ ] Cost forecasting + auto-remediation — multi-month spend projection from Infracost trend data per stack and per org; alerts when projected month-end spend exceeds the budget threshold; optional `block-on-forecast` mode rejects apply when the projection would breach budget; pairs with the v0.9.0 per-resource cost breakdown
