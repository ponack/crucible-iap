...
- [x] Cold Forge / Hot Forge theme switcher — two named forge themes selectable from the sidebar; Cold Forge (teal-slate, hue 185 OKLCH, `#2DD4BF` accent) and Hot Forge (copper-amber, hue 42 OKLCH, `#D4883C` accent, deep warm near-black page background); each combines independently with dark / light mode for four total palette combinations; selection persists to `localStorage` with anti-FOUC protection on hard reload
