...
- [x] Light-theme form input contrast fix (v0.9.9) — closes #384. Thirteen pages carried their own `:global(.field-input)` block with a hardcoded `color: #fff`, overriding the theme-aware utility in `app.css`; in light theme every filled control rendered white-on-white (Branch=`main`, Project root=`.` on `/stacks/new` were unreadable), and because the rules were global whichever page loaded first leaked its copy app-wide. Removed all thirteen duplicates so `app.css` is the single source of truth, promoted `field-label` to a shared utility, bumped placeholder contrast (`zinc-600` → `zinc-500`), and replaced the last hardcoded `#fff` / zinc hex in `.kbd` and the DAG hover state with theme variables. Also pins `staticcheck` to v0.7.0 in CI — `@latest` now resolves to v0.8.x which requires Go ≥ 1.26 while the lint job runs Go 1.25, so every PR was failing at linter install
- [x] MinIO image sunset stopgap (v0.9.10) — MinIO became a source-only distribution on 2025-10-15 and deleted its published containers from Docker Hub **and** quay.io ([minio/minio#21647](https://github.com/minio/minio/issues/21647)); `minio/minio:latest` stopped resolving, so every fresh `docker compose up` died at image pull, and `bitnami/minio` was already gone as a fallback. Existing deployments kept running on cached images but could neither pull nor upgrade — and because the final public release (`RELEASE.2025-10-15T17-29-55Z`) *was itself the CVE fix*, anyone on an older cached image was stranded unpatched. Both compose files now pin `coollabsio/minio:RELEASE.2025-10-15T17-29-55Z`, a community rebuild of that final release, verified drop-in: identical entrypoint and env contract, `minio` + `mc` binaries present, `mc ready local` healthcheck green, S3 round-trip clean, amd64 + arm64. No application change was needed — `minio-go` is a plain S3 SDK and the storage layer uses only standard S3 verbs, so any S3-compatible server works. **Deliberately a stopgap:** the mirror is frozen at October 2025 with no future security fixes; operators who would rather not run a frozen image can point `MINIO_ENDPOINT` at external S3 (AWS S3, Garage, RustFS, SeaweedFS, Ceph RGW) and drop the `minio` service entirely. Issue #387 tracks choosing a durable default
