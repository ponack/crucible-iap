...
| **Deployment** | Single `docker compose up` — Caddy, API, Worker, UI, PostgreSQL, MinIO, Prometheus, Grafana. Let's Encrypt TLS via Caddy or the `external-proxy` profile for nginx / Traefik / your own Caddy. Optional bundled Authentik IdP. Built-in Terraform module registry and stack templates. Optional `worker-agent` compose profile (or standalone `crucible-agent` binary) for running jobs on external hosts — configured via its own `.env.agent` file. |
