							...
							<td class="px-4 py-3">
								<div class="flex items-center gap-2">
									...
									{#if stack.upstream_count > 0 || stack.downstream_count > 0}
										{@const upNames = stack.upstream_stacks?.map((s: {id: string, name: string}) => s.name).join(', ')}
										{@const downNames = stack.downstream_stacks?.map((s: {id: string, name: string}) => s.name).join(', ')}
